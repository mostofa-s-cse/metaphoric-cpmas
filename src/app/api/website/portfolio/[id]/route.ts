...
const PATH = '/api/website/portfolio/[id]';

async function getHandler(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const data = await prisma.websitePortfolio.findUnique({ where: { id } });
...
async function putHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await prisma.websitePortfolio.update({ where: { id }, data: body });
