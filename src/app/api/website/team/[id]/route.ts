...
const PATH = '/api/website/team/[id]';

async function getHandler(request: NextRequest, { params }: { params: { id: string } }) {
  const { id } = params;
  const data = await prisma.websiteTeam.findUnique({ where: { id } });
...
async function putHandler(request: Request, { params }: { params: { id: string } }) {
  ...
  const data = await prisma.websiteTeam.update({ where: { id }, data: body });
...
async function deleteHandler(request: Request, { params }: { params: { id: string } }) {
  ...
  const { id } = params;
  await prisma.websiteTeam.delete({ where: { id } });
