...
async function getHandler(request: NextRequest) {
  const data = await prisma.websiteTeam.findMany({ orderBy: { order: 'asc' } });
  return apiSuccess(data, 'team retrieved successfully', PATH);
}
...
async function postHandler(request: Request) {
  ...
  const data = await prisma.websiteTeam.create({ data: body });
