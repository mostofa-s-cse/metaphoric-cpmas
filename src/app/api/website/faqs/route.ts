...
async function getHandler(request: NextRequest) {
  const data = await prisma.websiteFAQ.findMany({ orderBy: { order: 'asc' } });
  return apiSuccess(data, 'faqs retrieved successfully', PATH);
}
...
async function postHandler(request: Request) {
  ...
  const data = await prisma.websiteFAQ.create({ data: body });
