...
const PATH = '/api/website/testimonials';

async function getHandler(request: NextRequest) {
  const data = await prisma.websiteTestimonial.findMany({ orderBy: { order: 'asc' } });
  return apiSuccess(data, 'testimonials retrieved successfully', PATH);
}
...
async function postHandler(request: Request) {
  ...
  const data = await prisma.websiteTestimonial.create({ data: body });
