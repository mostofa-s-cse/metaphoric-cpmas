...
const PATH = '/api/website/testimonials/[id]';

async function getHandler(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const data = await prisma.websiteTestimonial.findUnique({ where: { id } });
...
async function putHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await prisma.websiteTestimonial.update({ where: { id }, data: body });
