...
const PATH = '/api/website/trust';

async function getHandler(request: NextRequest) {
  const data = await prisma.websiteTrustBadge.findMany({ orderBy: { order: 'asc' } });
  return apiSuccess(data, 'trust retrieved successfully', PATH);
}
...
async function postHandler(request: Request) {
  ...
  const data = await prisma.websiteTrustBadge.create({ data: body });
