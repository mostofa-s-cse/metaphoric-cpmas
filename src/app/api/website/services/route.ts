...
const PATH = '/api/website/services';

async function getHandler(request: NextRequest) {
  const data = await prisma.websiteService.findMany({ orderBy: { order: 'asc' } });
  return apiSuccess(data, 'services retrieved successfully', PATH);
}
...
async function postHandler(request: Request) {
  ...
  const data = await prisma.websiteService.create({ data: body });
