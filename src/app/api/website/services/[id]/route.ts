...
const PATH = '/api/website/services/[id]';

async function getHandler(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const data = await prisma.websiteService.findUnique({ where: { id } });
...
async function putHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await prisma.websiteService.update({ where: { id }, data: body });
