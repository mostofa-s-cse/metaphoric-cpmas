...
import { useGetTestimonialsQuery, useAddTestimonialMutation, useUpdateTestimonialMutation, useDeleteTestimonialMutation } from '@/store/api/websiteApi';
import { Modal } from '@/components/ui/Modal';
import { useToast } from '@/hooks/useToast';

export function TestimonialsTab() {
  const { data: items = [], isLoading } = useGetTestimonialsQuery();
  const [addItem, { isLoading: isAdding }] = useAddTestimonialMutation();
  const [updateItem, { isLoading: isUpdating }] = useUpdateTestimonialMutation();
  const [deleteItem] = useDeleteTestimonialMutation();
  const toast = useToast();

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState<any>({"isActive":true,"order":0,"clientName":"","clientRole":"","reviewText":""});

  const handleOpenNew = () => {
    setEditingId(null);
    setFormData({"isActive":true,"order":0,"clientName":"","clientRole":"","reviewText":""});
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await toast.handlePromise(promise, {
        successMessage: editingId ? 'Testimonial updated successfully' : 'Testimonial added successfully',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-semibold text-slate-200">Manage Testimonials</h2>
      ...
      <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit Item' : 'Add Item'} size="lg">
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" name="clientName" value={formData.clientName} onChange={handleChange} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-cyan-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Client Role / Company</label>
            <input type="text" name="clientRole" value={formData.clientRole} onChange={handleChange} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-cyan-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Review Content</label>
            <textarea name="reviewText" value={formData.reviewText} onChange={handleChange} rows={3} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 resize-none"></textarea>
          </div>
