...
import { useGetServicesQuery, useAddServiceMutation, useUpdateServiceMutation, useDeleteServiceMutation } from '@/store/api/websiteApi';
import { Modal } from '@/components/ui/Modal';
import { useToast } from '@/hooks/useToast';

export function ServicesTab() {
  const { data: items = [], isLoading } = useGetServicesQuery();
  const [addItem, { isLoading: isAdding }] = useAddServiceMutation();
  ...
  const [formData, setFormData] = useState<any>({"isActive":true,"order":0,"title":"","description":"","imageUrl":""});

  const handleOpenNew = () => {
    setEditingId(null);
    setFormData({"isActive":true,"order":0,"title":"","description":"","imageUrl":""});
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await toast.handlePromise(promise, {
        successMessage: editingId ? 'Service updated successfully' : 'Service added successfully',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-semibold text-slate-200">Manage Services</h2>
      ...
      <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit Item' : 'Add Item'} size="lg">
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea name="description" value={formData.description} onChange={handleChange} rows={3} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 resize-none"></textarea>
          </div>
          <div>
