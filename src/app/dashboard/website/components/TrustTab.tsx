...
import { useGetTrustBadgesQuery, useAddTrustBadgeMutation, useUpdateTrustBadgeMutation, useDeleteTrustBadgeMutation } from '@/store/api/websiteApi';
import { Modal } from '@/components/ui/Modal';
import { useToast } from '@/hooks/useToast';

export function TrustTab() {
  const { data: items = [], isLoading } = useGetTrustBadgesQuery();
  const [addItem, { isLoading: isAdding }] = useAddTrustBadgeMutation();
  const [updateItem, { isLoading: isUpdating }] = useUpdateTrustBadgeMutation();
  const [deleteItem] = useDeleteTrustBadgeMutation();
  const toast = useToast();

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState<any>({"isActive":true,"order":0,"name":"","type":"","imageUrl":""});

  const handleOpenNew = () => {
    setEditingId(null);
    setFormData({"isActive":true,"order":0,"name":"","type":"","imageUrl":""});
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await toast.handlePromise(promise, {
        successMessage: editingId ? 'Trust badge updated successfully' : 'Trust badge added successfully',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-semibold text-slate-200">Manage Trust</h2>
      ...
      <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit Item' : 'Add Item'} size="lg">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Badge Name</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Type (AWARD / CERTIFICATION / PARTNER_LOGO)</label>
            <input type="text" name="type" value={formData.type} onChange={handleChange} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-cyan-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Badge Image</label>
            <div className="flex items-center gap-4">
